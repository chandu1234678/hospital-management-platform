...
const INITIAL_QUEUE = [
  { token: 'T-001', name: 'Ananya Rao', type: 'OPD', doctor: 'Dr. Deepthi K.', wait: '5 min', status: 'Called' },
  { token: 'T-002', name: 'Ravi Shankar', type: 'OPD', doctor: 'Dr. R. Kumar', wait: '12 min', status: 'Waiting' },
  { token: 'T-003', name: 'Meena Pillai', type: 'Emergency', doctor: 'Dr. S. Williams', wait: '2 min', status: 'Waiting' },
  { token: 'T-004', name: 'Suresh Nair', type: 'OPD', doctor: 'Dr. A. Deshmukh', wait: '25 min', status: 'Waiting' },
  { token: 'T-005', name: 'Kavitha Reddy', type: 'OPD', doctor: 'Dr. M. Iyer', wait: '30 min', status: 'Waiting' },
  { token: 'T-006', name: 'Deepak Joshi', type: 'OPD', doctor: 'Dr. V. Nair', wait: '38 min', status: 'Done' },
]
...
const TYPE_STYLE = {
  OPD:       'bg-slate-100 text-slate-600',
    ...
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Live OPD token queue — {waiting} waiting</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                {['Token', 'Patient', 'Type', 'Doctor', 'Wait Time', 'Status', 'Actions'].map(h => (
              ...
              {queue.map(q => (
                <tr key={q.token} className={`transition-colors ${q.status === 'Called' ? 'bg-[#0F4C81]/5' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${TYPE_STYLE[q.type]}`}>{q.type}</span>
                  </td>
                  <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{q.doctor}</td>
                  <td className="px-4 py-3 text-slate-600">{q.wait}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {q.status !== 'Done' && (
                      <button onClick={() => markDone(q.token)}
                        className="text-green-600 text-xs font-semibold hover:underline inline-flex items-center gap-1">
