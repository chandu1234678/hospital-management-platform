    ...
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Today's schedule — {new Date().toLocaleDateString('en-IN', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'waiting', 'in progress', 'scheduled', 'completed', 'cancelled'].map(f => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50">
              <tr>
                {['ID', 'Patient', 'Doctor', 'Department', 'Time', 'Type', 'Status', 'Actions'].map(h => (
                ...
                <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">{a.age}y</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-sm text-slate-600">{a.doctor}</td>
                  <td className="px-5 py-4 text-sm text-slate-500">{a.dept}</td>
                  <td className="px-5 py-4 text-sm font-medium">{a.time}</td>
                  <td className="px-5 py-4 text-xs text-slate-500">{a.type}</td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => toast.success(`Viewing appointment ${a.id}`)}
